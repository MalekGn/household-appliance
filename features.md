...
| **Licensing / certification** | ✅ | Machine-bound, time-limited activation. The app refuses to run without a valid, Ed25519-signed license file (`license.psl`) bound to this machine's fingerprint and a due date; verified in Rust against an embedded public key. Invalid/missing/expired → the DB is left unmanaged (all data commands fail) and a full-screen activation lock shows the machine ID + import button. Anti-clock-rollback guard. Licenses are issued offline with the `tools/licensegen` CLI (see README → Licensing). |
...
| **Rapports (Reports)** | ✅ | Period-scoped synthesis: quick presets (this month / this year / all) + From–To pickers, KPI row (sales & collected over the period, current outstanding & overdue snapshot), continuous month-by-month breakdown and per-client breakdown (both sortable), and a full CSV export of the report (native OS Save As dialog on the desktop app — written backend-side via a Tauri command — with a browser download fallback in dev/preview). |
...
- **Unit tests:** `npm test` (Vitest) — co-located in `src/**`, covering the finance helpers (`src/lib/finance.test.ts`), the overdue data/sort logic (`src/views/impayes-overdue.test.ts`), and the license store/gate (`src/stores/license.test.ts`). Rust-side, `cd src-tauri && cargo test` covers the license verifier (`src/license.rs`): signature, machine binding, expiry window, and clock-rollback branches with a test keypair.
- **Integration tests:** `npm run test:integration` (Vitest, `vitest.integration.config.ts`) — suites in `tests/integration/` drive the real `api` facade against the in-memory backend across multi-command flows: the purchase → schedule → payment lifecycle, overdue/dashboard/cascade-delete consistency, and the licensing gate (lock → import → unlock, plus expired-stays-locked via `license-gate.integration.test.ts`). Kept out of the default unit run so they stay opt-in.
- **End-to-end tests:** `npm run test:e2e` — self-contained Playwright suite (`tests/e2e/run.mjs`) drives the real app in headless Chromium against the in-memory mock backend. Spawns its own Vite server on port 5199 and pins the browser locale to French for determinism. Covers app-shell render, dashboard KPIs, full sidebar navigation, client list + create flow, purchase list + search, the overdue (impayés)/alertes/rapports pages, and the licensing activation gate (invalid → lock screen, import → unlock into the app shell, expired → stays locked). Failures capture a full-page screenshot under `tests/e2e/artifacts/`.
